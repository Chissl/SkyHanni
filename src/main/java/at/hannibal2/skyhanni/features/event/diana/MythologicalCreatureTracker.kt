...
object MythologicalCreatureTracker {
    ...
    fun onConfigFix(event: ConfigUpdaterMigrator.ConfigFixEvent) {
        event.transform(115, "#profile.diana.mythologicalMobTracker", ::fixData)
        event.transform(115, "#profile.diana.mythologicalMobTrackerPerElection", ::fixPastData)

        val timedTrackerData: TimedTrackerData<Data> = TimedTrackerData { Data() }

        event.transform(113, "#profile.diana.mythologicalMobTracker") { entry ->
            timedTrackerData.createEntry(DisplayMode.TOTAL, "total", ConfigManager.gson.fromJson<Data>(entry))
            entry
        }
        event.transform(113, "#profile.diana.mythologicalMobTrackerPerElection") { entry ->
        ...
        event.add(113, "#profile.diana.timedMythologicalMobTracker") {
